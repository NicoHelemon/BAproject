...
class GrabCut:
    ...
    def assign_GMMs_components(self):
        ...
        u = self.bgd_gmm.which_component(
            self.img[self.bgd_indexes])
        v = self.fgd_gmm.which_component(
            self.img[self.fgd_indexes])
        print(type(u))
        print(len(u) + len(v))
        print(np.unique(u), np.unique(v))
        print(len(self.comp_idxs))
        print(self.comp_idxs.shape)
        print(np.unique(self.comp_idxs))
        rr = np.zeros(self.comp_idxs.shape)
        gg = np.zeros(self.comp_idxs.shape)
        bb = np.zeros(self.comp_idxs.shape)
        rr[self.bgd_indexes] = self.bgd_gmm.which_component(
            self.img[self.bgd_indexes]) * 63
        bb[self.fgd_indexes] = self.fgd_gmm.which_component(
            self.img[self.fgd_indexes]) * 63
        rgb = np.stack([rr, gg, bb], axis=2)
        im.show(rgb)
